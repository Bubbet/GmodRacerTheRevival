...
surface.CreateFont("coolvetica", 70, 500, true, false, "3DScoreboardHead");
surface.CreateFont("coolvetica", 40, 500, true, false, "3DScoreboardText");
surface.CreateFont("LCD", 40, 700, true, false, "3DScoreboardText_LCD");
	...
	*/
	...
	cam.Start3D2D(self:GetPos(), Angle, 1 / Scale);
		...
		if GetGlobalInt('GamemodeType') == 2 or !GetGlobalBool("CurrentlyRacing") or !Leader then			
			...
			draw.SimpleText("< PulsarEffect.Com >", "3DScoreboardHead", self.X + self.W * .5, self.Y + self.H * .5, GreyColor, 1, 1);
		else
			surface.SetDrawColor(255, 255, 255, 255);
