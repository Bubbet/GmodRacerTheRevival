Part.Name = "Gravity Cannon ( VIP ONLY )";
Part.Class = "Mounted Cannon";
Part.ClassLevel = 2;
Part.Cost = 0;
Part.Icon = "cannon";
Part.Description = "Fire with Right Mouse Click.";
Part.Sellable = true;
Part.RequiredModels = {}
Part.RequiresAccess = 4;

// Engine Details
Part.AddedWeight = 50; // Weight of the part.
...
function Part.Place ( Jeep, Forward, Back, Right, Left, Up, Down )
	...
	Jeep.LastCannonFire = 0;
	
	Entity:SetPos(Jeep:GetPos() + (Back * 40) + Vector(0, 0, 83))
	Entity:SetAngles(Jeep:GetAngles() + Angle(0, 0, -95));
	Entity:SetModel("models/props_trainstation/trashcan_indoor001b.mdl");
	Entity:SetParent(Jeep);
	Entity:Spawn();
	
	Jeep.EMPCannon = Entity;
