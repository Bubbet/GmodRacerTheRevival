...
function GM:PlayerBindPress ( Player, Bind, Pressed )
	if Bind == "+use" or Bind == "-use" or string.find(Bind, "slot") then
		return true;
	elseif Pressed and Bind == "+duck" and ValidEntity(Player:GetVehicle()) then
	...
	elseif Bind == "+jump" and Player:GetUsedPart("Hand Brake") == 0 and ValidEntity(Player:GetVehicle()) then
		return true;
