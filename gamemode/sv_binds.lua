...
function GM:KeyPress ( Player, Key ) 
	...
	elseif Key == IN_WALK and Player:InVehicle() and Player:GetUsedPart("Horn") != 0 and self.PlayerTables[Player].LastHornBlast + HornDelay <= CurTime() then
		...
		elseif Player:GetUsedPart("Horn") == 5 then
			--PEAchievements.Update(Player, "GMod Racer: Rolling the Rick", 1);
	...
	elseif CurTime() >= self.RaceStartTime + 5 and Key == IN_RELOAD and Player:InVehicle() and Player:GetVehicle().RocketsRemaining != nil and (Player:IsRacing() or Player:GetLevel() <= 1) then
	...
	elseif CurTime() >= self.RaceStartTime + 5 and Key == IN_SCORE and Player:InVehicle() and Player:GetVehicle().MinesRemaining != nil and (Player:IsRacing() or Player:GetLevel() <= 1) then
		if Player:GetVehicle().MinesRemaining > 0 then
			if Player:GetVehicle().Mines[Player:GetVehicle().MinesRemaining] then
				...
				local Jeep = Player:GetVehicle();
				--PEAchievements.Update(Player, "GMod Racer: Bomber", 1);
	...
	elseif CurTime() >= self.RaceStartTime + 5 and Key == IN_ATTACK2 and Player:InVehicle() and (Player:GetVehicle().Cannon != nil or Player:GetVehicle().EMPCannon != nil) and (Player:IsRacing() or Player:GetLevel() <= 1) then
		if Player:GetVehicle().LastCannonFire + 15 < CurTime() then
			local Mag = 1;
			local Ent;
			if Player:GetVehicle().Cannon != nil then
				Ent = Player:GetVehicle().Cannon;
				timer.Simple(2.3, self.FireLaser, Player:GetVehicle().Cannon, Player)
				Player:GetVehicle().Cannon:EmitSound(Sound('gmracer/laser_reload.wav'), 500);
			else
				Ent = Player:GetVehicle().EMPCannon;
				timer.Simple(2.3, self.FireEMP, Player:GetVehicle().EMPCannon, Player)
...
function GM.LaserReadyBeam ( )
	...
	for k, v in pairs(player.GetAll()) do
		if v:InVehicle() and (v:GetVehicle().Cannon != nil or v:GetVehicle().EMPCannon != nil) and v:GetVehicle().LastCannonFire + 15 < CurTime() and (v:IsRacing() or v:GetLevel() <= 1) then
			local effectdata = EffectData()
	...
	for k, v in pairs(ents.GetAll()) do
		if v:IsVehicle() then
			...
			if Distance < 750 then
				...
				v:GetPhysicsObject():ApplyForceCenter((v:GetPos() - HitPos):Normalize() * Power);
			end
		end
	end
...
function GM.SetFalse ( Player ) Player:SetNetworkedBool("IsTurboing", false); end
...
function GM.ExtraPlayerSpawn ( Player ) if Player and Player:IsValid() and Player:IsPlayer() then Player:Freeze(false); end end
...
function GM:PlayerInitialSpawn( Player )
	...
	for k, v in pairs(self.PartsTable) do	
		self.PlayerStats[Player][k] = Player:GetPData("GMRacer_PartUsing_" .. k, "0");
		
		Num = Num + .05
		if self.PlayerStats[Player][k] != "0" then
			timer.Simple(Num, PassInfo, Player, k, self.PlayerStats[Player][k]);
		end
	end
	
	timer.Simple(1, SendCash, Player);
	
	local Success = Player:GetPData("Money")
	
	if Success then
		Player:AddCash(Success)
		Msg("Loaded player with Cash: " .. Success .. "\n");
	else
		Player:SetPData("Money", 0)
	...
	--[[
	local Results, Success, Error = MySQLQuery(SiteDatabaseConnection, "SELECT * FROM `gmr_records` WHERE `SteamID`='" .. Player:SteamID() .. "' AND `Map`='" .. game.GetMap() .. "'", mysql.QUERY_FIELDS);
	if Results[1] then
		Player:SetNetworkedInt("MapRecord", tonumber(Results[1]['Time']));
	else
		local Results, Success, Error = MySQLQuery(SiteDatabaseConnection, "INSERT INTO `gmr_records` (`SteamID`, `Time`, `Name`, `Map`) VALUES ('" .. Player:SteamID() .. "', 1337, '" .. Player:Name() .. "', '" .. game.GetMap() .. "')");
...
function GM:PlayerDeath ( Player )
	...
	if Player:IsRacing() then
		...
		if GetGlobalInt('GamemodeType') == 2 then
			local CashToGrant = GAMEMODE.NumberPeopleFinished * 50;
			...
			local KillsCash = Player:GetNetworkedInt("RoundKills") * 50;
			...
			if Player:GetLevel() < 4 then
				...
				Player:PrintMessage(HUD_PRINTTALK, "You earned $" .. Extra .. " ( 25% ) bonus cash for being an Admin!");
