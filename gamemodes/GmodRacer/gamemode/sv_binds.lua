...
function GM:KeyPress ( Player, Key ) 
	...
	elseif CurTime() >= self.RaceStartTime + 5 and Key == IN_RELOAD and Player:InVehicle() and Player:GetVehicle().RocketsRemaining != nil and (Player:IsRacing() or Player:Team() <= 1) then
	...
	elseif CurTime() >= self.RaceStartTime + 5 and Key == IN_SCORE and Player:InVehicle() and Player:GetVehicle().MinesRemaining != nil and (Player:IsRacing() or Player:Team() <= 1) then
	...
	elseif CurTime() >= self.RaceStartTime + 5 and Key == IN_ATTACK2 and Player:InVehicle() and (Player:GetVehicle().Cannon != nil or Player:GetVehicle().EMPCannon != nil) and (Player:IsRacing() or Player:Team() <= 1) then
...
function GM.LaserReadyBeam ( )
	...
	for k, v in pairs(player.GetAll()) do
		if v:InVehicle() and (v:GetVehicle().Cannon != nil or v:GetVehicle().EMPCannon != nil) and v:GetVehicle().LastCannonFire + 15 < CurTime() and (v:IsRacing() or v:Team() <= 1) then
			local effectdata = EffectData()
